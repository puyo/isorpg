...
int main(int argc, char argv[]) {
    ...
    allegro_init();
    {
        ...
        set_gfx_mode(GFX_AUTODETECT, 1024, 800, 0, 0);

        map.make_tiles();

        BITMAP_PTR buffer = bitmap_ptr(create_bitmap(SCREEN_W, SCREEN_H));
        BITMAP_PTR buffer2 = bitmap_ptr(create_bitmap(SCREEN_W, SCREEN_H));
        clear(buffer.get());
        clear(buffer2.get());
        ...
        do {
            ...
            textprintf_ex(buffer.get(), font, 0, 0, -1, -1, "FPS: %u", fps);
            //vsync();
