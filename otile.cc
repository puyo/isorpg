...
OTILE::OTILE(uint id, DATAFILE * itilesdata): myextra_xoff(0), myextra_yoff(0) {
...
void OTILE::construct(uint id, DATAFILE * itilesdata) {
    ...
    int tw = 16, th = 8;
    myimage = bitmap_ptr(create_bitmap(tw, th));
    ...
    clear(myimage.get());
    if (iimage2 != NULL)
        blit(iimage2, myimage.get(), ix2, iy2, 0, 0, tw, th);

    if (iimage1 != NULL)
        masked_blit(iimage1, myimage.get(), ix1, iy1, 0, 0, tw, th);
    ...
    if (extrah > 0) {
        destroy_bitmap(myextra.get());
        myextra = bitmap_ptr(create_bitmap(tw, extrah));
        clear(myextra.get());
        blit(iimage1, myextra.get(), ix1, 0, 0, 0, tw, extrah);
        myextra_xoff = 0;
        myextra_yoff = -extrah;
    }
...
void OTILE::draw(BITMAP * dest, int x, int y) {
    ...
    blit(myimage.get(), dest, 0, 0, x, y, myimage->w, myimage->h);
    if (myextra.get() != 0)
        draw_sprite(dest, myextra.get(), x + myextra_xoff, y + myextra_yoff);
}
